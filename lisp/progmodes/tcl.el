...
(defvar tcl-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c C-f") 'tcl-load-file)
    (define-key map (kbd "C-c C-t") 'inferior-tcl)
    (define-key map (kbd "C-c C-x") 'tcl-eval-region)
    (define-key map (kbd "C-c C-s") 'switch-to-tcl)
    map)
