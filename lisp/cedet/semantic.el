...
(defvar semantic-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c , J") #'semantic-complete-jump)
    (define-key map (kbd "C-c , m") #'semantic-complete-jump-local-members)
    (define-key map (kbd "C-c , g") #'semantic-symref-symbol)
    (define-key map (kbd "C-c , G") #'semantic-symref)
    (define-key map (kbd "C-c , p") #'senator-previous-tag)
    (define-key map (kbd "C-c , n") #'senator-next-tag)
    (define-key map (kbd "C-c , u") #'senator-go-to-up-reference)
    (define-key map (kbd "C-c , SPC") #'semantic-complete-analyze-inline)
    (define-key map (kbd "C-c , C-w") #'senator-kill-tag)
    (define-key map (kbd "C-c , M-w") #'senator-copy-tag)
    (define-key map (kbd "C-c , C-y") #'senator-yank-tag)
    (define-key map (kbd "C-c , r") #'senator-copy-tag-to-register)
    (define-key map (kbd "C-c , ,") #'semantic-force-refresh)
    (define-key map [?\C-c ?, up] #'senator-transpose-tags-up)
    (define-key map [?\C-c ?, down] #'senator-transpose-tags-down)
    (define-key map (kbd "C-c , l") #'semantic-analyze-possible-completions)
