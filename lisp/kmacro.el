...
(defvar kmacro-keymap
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-s") #'kmacro-start-macro)
    (define-key map (kbd "C-k") #'kmacro-end-or-call-macro-repeat)
    ...
    (define-key map (kbd "C-v") #'kmacro-view-macro-repeat)
    (define-key map (kbd "C-d") #'kmacro-delete-ring-head)
    (define-key map (kbd "C-t") #'kmacro-swap-ring)
    (define-key map (kbd "C-l") #'kmacro-call-ring-2nd-repeat)

    ;; macro counter
    (define-key map (kbd "C-f") #'kmacro-set-format)
    (define-key map (kbd "C-c") #'kmacro-set-counter)
    (define-key map (kbd "C-i") #'kmacro-insert-counter)
    (define-key map (kbd "C-a") #'kmacro-add-counter)

    ;; macro editing
    (define-key map (kbd "C-e") #'kmacro-edit-macro-repeat)
  ...
  (if (or defining-kbd-macro executing-kbd-macro)
	...
	(let ((ch (aref key-seq 0)))
	  (if (and (integerp ch)
		   (or (and (>= ch ?0) (<= ch ?9))
		       (and (>= ch ?A) (<= ch ?Z))))
	      (setq key-seq (concat (kbd "C-x C-k") key-seq)
		    ok t))))
      (when (and (not (equal key-seq (kbd "C-g")))
		 (or ok
