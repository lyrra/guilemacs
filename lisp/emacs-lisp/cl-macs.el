...
(defun cl--parse-loop-clause ()		; uses loop-*
  (let ((word (pop cl--loop-args))
	...
	(while
		...
		(let ((temp-vec (intern-gensym "--cl-vec--"))
		      (temp-idx (intern-gensym "--cl-idx--")))
		...
		(let ((ref (or (memq (car cl--loop-args) '(in-ref of-ref))
		      ...
		      (temp-idx
                       (if (eq (car cl--loop-args) 'using)
                         ...
                         (intern-gensym "--cl-idx--"))))
		  (push (list temp-seq seq) loop-for-bindings)
		  (push (list temp-idx 0) loop-for-bindings)
		  (if ref
                      (let ((temp-len (intern-gensym "--cl-len--")))
...
\(fn ((FUNC ARGLIST BODY...) ...) FORM...)"
  ...
  (let ((binds ()) (newenv macroexpand-all-environment))
    (dolist (binding bindings)
      (let* ((var (intern-gensym (format "--cl-%s--" (car binding))))
...
slots skipped by :initial-offset may appear in the list."
  ...
  (let* ((class (cl--struct-get-class struct-type)))
    ;; Built-in types don't have slot info, return nil
    (when (cl--struct-class-p class)
      (let* ((slots (cl--struct-class-slots class))
