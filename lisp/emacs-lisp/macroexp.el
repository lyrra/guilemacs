...
\(fn (VAR LIST) BODY...)"
  ...
  (let ((var (car var+list))
	(list (cadr var+list))
	(shared (intern-gensym "shared"))
	(unshared (intern-gensym "unshared"))
	(tail (intern-gensym "tail"))
	(new-el (intern-gensym "new-el")))
    `(let* ((,shared ,list)
	    (,unshared nil)
	    (,tail ,shared)
	    ,var ,new-el)
...
cases where EXP is a constant."
  ...
  (let ((bodysym (intern-gensym "body"))
        (expsym (intern-gensym "exp")))
    `(let* ((,expsym ,exp)
            (,sym (if (funcall #',(or test #'macroexp-const-p) ,expsym)
                      ,expsym (intern-gensym ,(symbol-name sym))))
