...
(defvar calc-alg-ent-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-m") 'calcAlg-enter)
    (define-key map (kbd "C-j") 'calcAlg-enter)
    map)
...
(defun calc-do-alg-entry (&optional initial prompt no-normalize history)
  (let* ((calc-buffer (current-buffer))
	 ...
	 (calc-alg-exp 'error))
    (define-key calc-alg-ent-map "\x1b" nil)
    (if (eq calc-algebraic-mode 'total)
	(define-key calc-alg-ent-map "\x1b" calc-alg-ent-esc-map)
      (define-key calc-alg-ent-map "\x1b+" 'calcAlg-plus-minus)
      (define-key calc-alg-ent-map "\x1bm" 'calcAlg-mod)
      (define-key calc-alg-ent-map "\x1b=" 'calcAlg-equals)
      (define-key calc-alg-ent-map "\x1b\r" 'calcAlg-equals)
      (define-key calc-alg-ent-map "\x1bp" 'previous-history-element)
      (define-key calc-alg-ent-map "\x1bn" 'next-history-element)
      (define-key calc-alg-ent-map "\x1b%" 'self-insert-command))
