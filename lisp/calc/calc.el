...
(defvar calc-mode-map
  (let ((map (make-keymap)))
    ...
    (define-key map (kbd "M-%") 'calc-percent)
    ...
    (define-key map "\"" 'calc-auto-algebraic-entry)
    (define-key map (kbd "TAB") 'calc-roll-down)
    (define-key map (kbd "M-TAB") 'calc-roll-up)
    (define-key map (kbd "C-x C-t") 'calc-transpose-lines)
    (define-key map (kbd "C-m") 'calc-enter)
    (define-key map (kbd "M-C-m") 'calc-last-args-stub)
    (define-key map (kbd "C-j") 'calc-over)
    (define-key map (kbd "C-y") 'calc-yank)
    ...
    (mapc (lambda (x) (define-key map (char-to-string x) 'calc-missing-key))
          (concat "ABCDEFGHIJKLMNOPQRSTUVXZabcdfghjkmoprstuvwxyz"
                  ":\\|!()[]<>{},;=~`\x0b\x17"))
    (define-key map (kbd "M-w") 'calc-missing-key)
    (define-key map (kbd "M-k") 'calc-missing-key)
    (define-key map (kbd "M-C-w") 'calc-missing-key)
