...
(deftestf 'logcount (1)
  (el-expr `(let ((num ,(ash 1 (random 128))))
              (print (logcount num)))))

(let ((num (ash 1 (random 128))))
  (deftestf 'plus1 ((+ 1 num))
    (el-expr `(print ("1+" ,num))))
  (deftestf 'plus1 ((+ 1 num))
    (el-expr `(let ((num ,num))
                (print ("1+" num))))))
...
(let ((num (ash 1 (random 128))))
  (deftestf 'lognot ((lognot num))
    (el-expr `(print (lognot ,num))))
  (deftestf 'lognot ((lognot num))
    (el-expr `(let ((num ,num))
                (print (lognot num))))))
...
(let ((num1 (ash 1 (random 128)))
  ...
  (deftestf 'remainder ((remainder num1 num2))
    (el-expr `(let ((num1 ,num1)
                    (num2 ,num2))
                (print (% num1 num2))))))

(define (test-modulo a b)
  (deftestf 'modulo ((modulo a b))
    (el-expr `(print (mod ,a ,b))))
  (deftestf 'modulo ((modulo a b))
    (el-expr `(let ((num1 ,a)
                    (num2 ,b))
                (print (mod num1 num2))))))
