;;; test-m34-imp1.el --- M34 imp-1 (dispnew.c display/poll callers) suite.
;;;
;;; Pins the M34 imp-1 port (brief.org): the decision logic of the six
;;; live display/poll call sites in src/dispnew.c moved into the new
;;; module (emacs display).  src/dispnew.c now calls five static
;;; dispatchers; no stub retires.
;;;
;;; Wraps test/keyboard/test-m34-imp1.scm -- the Scheme audit corpus.
...
(princ "=== m34 imp-1 (dispnew.c display/poll callers) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m34-imp1.scm" dir))
       (load-error nil))
      ...
      (let* ((name (car result))
        ...
        (cond
         (ok
          ...
          (eval `(ert-deftest ,(intern (format "m34-imp1/%s" name)) ()
