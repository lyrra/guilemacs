;;; test-m33-imp1.el --- M33 imp-1 (term.c TTY help/mouse consumers) suite.
;;;
;;; Pins the M33 imp-1 port (brief.org): the decision logic of the TTY
;;; menu help/mouse paths in src/term.c moved into the new module (emacs
;;; terminal), and src/term.c now calls the C dispatchers
;;; tty_menu_clear_help / tty_menu_discard_mouse_events and the
;;; tty_menu_help_callback dispatcher.  The discard_mouse_events stub
;;; retired.
;;;
;;; Wraps test/keyboard/test-m33-imp1.scm -- the Scheme audit corpus.
...
(princ "=== m33 imp-1 (term.c TTY help/mouse consumers) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m33-imp1.scm" dir))
       (load-error nil))
      ...
      (let* ((name (car result))
        ...
        (cond
         (ok
          ...
          (eval `(ert-deftest ,(intern (format "m33-imp1/%s" name)) ()
