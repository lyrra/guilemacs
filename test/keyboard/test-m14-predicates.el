;;; test-m14-predicates.el --- M14 imp-4 test suite for the C->Scheme
;;; cutover of the 5 kbd-buffer predicates & drainers
;;;
;;; Wraps test/keyboard/test-m14-predicates.scm — the Scheme test
;;; corpus that proves the C thin dispatchers (readable_events,
;;; process_special_events, swallow_events, discard_mouse_events,
;;; kbd_buffer_events_waiting) reach their Scheme bodies in
;;; (emacs kbd-buffer).  See docs/m14-plan.org §imp-4.
;;;
;;; Loads the Scheme file via eval-scheme, then reads back
;;; `test-results` (list of (NAME STATUS) pairs) and reports each
;;; via princ.  Same harness as test-m14-bodies.el (M14 imp-2).

(princ "=== m14-predicates test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       (corpus (expand-file-name "test-m14-predicates.scm" dir)))
  (condition-case err
      (eval-scheme
       (format "(primitive-load %S)" corpus))
    (error (princ (format "M14P-CORPUS-LOAD-ERROR: %S\n" err)))))

;; Read each result back and report PASS/FAIL.
(let ((results (condition-case e
                   (eval-scheme "(reverse test-results)")
                 (error (princ (format "M14P-READBACK-ERROR: %S\n" e)) '())))
      (pass 0)
      (fail 0))
