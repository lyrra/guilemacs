;;; test-m30-imp5.el --- M30 imp-5 (close-out) audit suite.
;;;
;;; Pins the M30 imp-5 close-out (brief.org M30 imp-5): the getctag
;;; get/set pair moves to the C cell table (getctag is rooted first),
;;; and the other 47 remaining per-cell names stay C.  Every stay-C
;;; name must NOT be a table key.
;;;
;;; Wraps test/keyboard/test-m30-imp5.scm -- the Scheme proof corpus.
...
;;; silent 0 failed).  Same shape as test-m30-imp4.el.

(princ "=== m30-imp5 (close-out: ctag conversion + 47 stay-C names) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m30-imp5.scm" dir))
       (load-error nil))
  ;; The corpus reads the build tree (the static DEFUN scan), so tell it
  ;; where the repo root is.
  (eval-scheme (format "(define %%m30-root %S)" root))
  (condition-case err
      (eval-scheme
  ...
  (let* ((results
    ...
    (dolist (result results)
      (let* ((name (car result))
        ...
        (eval `(ert-deftest ,(intern (format "m30-imp5/%s" name)) ()
