;;; test-m32-imp6.el --- M32 imp-6 (close-out audit) suite.
;;;
;;; Pins the M32 imp-6 close-out (brief.org): the two stub retirements
;;; (cmd_error_internal, stuff_buffered_input) leave no caller,
;;; definition, or extern; the 6 migrated names have no C storage; and
;;; the anchored Job 1 counts match the brief table.
;;;
;;; Wraps test/keyboard/test-m32-imp6.scm -- the Scheme audit corpus.
...
(princ "=== m32 imp-6 (close-out audit) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m32-imp6.scm" dir))
       (load-error nil))
      ...
      (let* ((name (car result))
        ...
        (cond
         (ok
          ...
          ;; The corpus name is already namespaced (m32/imp6/...), so use
          ;; it verbatim; do not prepend a second m32-imp6/ prefix.
