;;; test-m34-imp3.el --- M34 imp-3 (window.c/fileio.c safe_run_hooks) suite.
;;;
;;; Pins the M34 imp-3 port (brief.org): the decision logic of the two
;;; live src/window.c and src/fileio.c call sites of the safe_run_hooks
;;; stub moved into the new modules (emacs window) and (emacs fileio).
;;; src/window.c and src/fileio.c now call one static dispatcher each;
;;; no stub retires.
;;;
;;; Wraps test/keyboard/test-m34-imp3.scm -- the Scheme audit corpus.
...
(princ "=== m34 imp-3 (window.c/fileio.c safe_run_hooks) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m34-imp3.scm" dir))
       (load-error nil))
      ...
      (let* ((name (car result))
        ...
        (cond
         (ok
          ...
          (eval `(ert-deftest ,(intern (format "m34-imp3/%s" name)) ()
