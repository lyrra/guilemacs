;;; test-m34-imp5.el --- M34 imp-5 (xdisp.c part-1 hook callers) suite.
;;;
;;; Pins the M34 imp-5 port (brief.org): the decision logic of the three
;;; live src/xdisp.c hook call sites moved into the new module
;;; (emacs xdisp).  src/xdisp.c now calls three static dispatchers; site
;;; S4 (the tool-bar event store) stays C (Option B); no stub retires and
;;; no DEFVAR_* site leaves C.
;;;
;;; Wraps test/keyboard/test-m34-imp5.scm -- the Scheme audit corpus.
...
(princ "=== m34 imp-5 (xdisp.c part-1 hook callers) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m34-imp5.scm" dir))
       (load-error nil))
      ...
      (let* ((name (car result))
        ...
        (cond
         (ok
          ...
          (eval `(ert-deftest ,(intern (format "m34-imp5/%s" name)) ()
