;;; test-m32-imp4.el --- M32 imp-4 (eval.c probably_quit + 2 names) suite.
;;;
;;; Pins the M32 imp-4 port (brief.org): the eval.c caller of
;;; process_pending_signals moved into Scheme as (emacs eval-main)
;;; probably-quit!; the C entry point probably_quit is now a thin
;;; dispatcher into it, keeping the process_quit_flag mechanism in C;
;;; and num-nonmacro-input-events + throw-on-input move out of C for a
;;; boot-loaded Scheme declaration.
;;;
;;; Wraps test/keyboard/test-m32-imp4.scm -- the Scheme audit corpus.
...
(princ "=== m32 imp-4 (eval.c probably_quit + 2 names) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m32-imp4.scm" dir))
       (load-error nil))
      ...
      (let* ((name (car result))
        ...
        (cond
         (ok
          ...
          ;; The corpus name is already namespaced (m32/imp4/...), so use
          ;; it verbatim; do not prepend a second m32-imp4/ prefix.
