;;; test-m30-imp4.el --- M30 imp-4 (plain Lisp_Object cell conversion) suite.
;;;
;;; Pins the M30 imp-4 change (brief.org M30 imp-4): the plain
;;; Lisp_Object cells move to the C cell table and the per-cell get/set
;;; DEFUNs are deleted.  The old accessor names stay, now defined by
;;; (emacs cell-accessors).
;;;
;;; Wraps test/keyboard/test-m30-imp4.scm -- the Scheme proof corpus.
...
;;; silent 0 failed).  Same shape as test-m30-imp3.el.

(princ "=== m30-imp4 (plain Lisp_Object cell conversion) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       (corpus (expand-file-name "test-m30-imp4.scm" dir))
       (load-error nil))
  (condition-case err
      (eval-scheme
  ...
  (let* ((results
    ...
    (dolist (result results)
      (let* ((name (car result))
        ...
        (eval `(ert-deftest ,(intern (format "m30-imp4/%s" name)) ()
