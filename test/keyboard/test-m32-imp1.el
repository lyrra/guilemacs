;;; test-m32-imp1.el --- M32 imp-1 (process wait decision path) suite.
;;;
;;; Pins the M32 imp-1 port (brief.org): the decision logic of the live
;;; wait_reading_process_output (src/process.c:5329) moved into the new
;;; module (emacs process-wait), and src/process.c now calls the two C
;;; dispatchers wait_signal_drain / wait_run_timers.  The system call
;;; stays C.
;;;
;;; Wraps test/keyboard/test-m32-imp1.scm -- the Scheme audit corpus.
...
(princ "=== m32 imp-1 (process wait decision path) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       ...
       (corpus (expand-file-name "test-m32-imp1.scm" dir))
       (load-error nil))
      ...
      (let* ((name (car result))
        ...
        (cond
         (ok
          ...
          (eval `(ert-deftest ,(intern (format "m32-imp1/%s" name)) ()
