;;; test-m27-single-kboard.el --- M27 imp-1 (emacs single-kboard) test suite.
;;;
;;; Covers the M27 imp-1 cutover (brief.org M27): the single-kboard
;;; family policy (not-single-kboard-state, push-kboard!, pop-kboard!,
;;; temporarily-switch-to-single-kboard!) moved out of src/keyboard.c
;;; into (emacs single-kboard).
;;;
;;; Wraps test/keyboard/test-m27-single-kboard.scm -- the Scheme test
...
(princ "=== m27-single-kboard test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       (corpus (expand-file-name "test-m27-single-kboard.scm" dir)))
  (condition-case err
      (eval-scheme
       (format "(primitive-load %S)" corpus))
    (error (princ (format "M27SK-CORPUS-LOAD-ERROR: %S\n" err)))))

;; Read each result back and report PASS/FAIL.
(let ((results (condition-case e
                   (eval-scheme "(reverse test-results)")
                 (error (princ (format "M27SK-READBACK-ERROR: %S\n" e)) '())))
      (pass 0)
      (fail 0))
