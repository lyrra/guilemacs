;;; test-m17-bodies.el --- M17 imp-2 test suite for the Scheme
;;; record-char procedure
;;;
;;; Wraps test/keyboard/test-m17-bodies.scm — the Scheme test corpus
;;; for record-char in (emacs recent-keys), a port of C record_char.
;;; See docs/m17-plan.org §imp-2 and brief.org.
;;;
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       (corpus (expand-file-name "test-m17-bodies.scm" dir)))
  (condition-case err
      (eval-scheme
       (format "(primitive-load %S)" corpus))
    (error (princ (format "M17B-CORPUS-LOAD-ERROR: %S\n" err)))))

;; Read each result back and report PASS/FAIL.
(let ((results (condition-case e
                   (eval-scheme "(reverse test-results)")
                 (error (princ (format "M17B-READBACK-ERROR: %S\n" e)) '())))
      (pass 0)
      (fail 0))
