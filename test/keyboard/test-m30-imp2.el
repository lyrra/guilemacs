;;; test-m30-imp2.el --- M30 imp-2 (plain bool cell conversion) suite.
;;;
;;; Pins the M30 imp-2 change (brief.org M30 imp-2): the plain bool
;;; cells move to the C cell table and the per-cell DEFUNs are deleted.
;;; The old accessor names stay, now defined by (emacs cell-accessors).
;;;
;;; Wraps test/keyboard/test-m30-imp2.scm -- the Scheme proof corpus.
...
;;; silent 0 failed).  Same shape as test-m30-imp1.el.

(princ "=== m30-imp2 (plain bool cell conversion) test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       (corpus (expand-file-name "test-m30-imp2.scm" dir))
       (load-error nil))
  (condition-case err
      (eval-scheme
  ...
  (let* ((results
    ...
    (dolist (result results)
      (let* ((name (car result))
        ...
        (eval `(ert-deftest ,(intern (format "m30-imp2/%s" name)) ()
