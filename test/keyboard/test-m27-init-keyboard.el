;;; test-m27-init-keyboard.el --- M27 imp-3 (emacs keyboard-init) test suite.
;;;
...
;;; (emacs keyboard-init) as init-keyboard!, leaving a thin C dispatcher
;;; that keeps only the current-kboard re-init, the sigaction installs
;;; and the signal / poll arms.
;;;
;;; Wraps test/keyboard/test-m27-init-keyboard.scm -- the Scheme test
;;; corpus.  Loads the Scheme file via eval-scheme, then reads back
;;; `test-results` (list of (NAME STATUS) pairs) and reports each via
;;; princ.  Same harness as test-m27-kboard-lifecycle.el.  See brief.org
;;; M27 imp-3.

(princ "=== m27-init-keyboard test suite ===\n")
...
(let* ((dir (file-name-directory (or load-file-name default-directory)))
       (corpus (expand-file-name "test-m27-init-keyboard.scm" dir)))
  (condition-case err
      (eval-scheme
       (format "(primitive-load %S)" corpus))
    (error (princ (format "M27IK-CORPUS-LOAD-ERROR: %S\n" err)))))

;; Read each result back and report PASS/FAIL.
(let ((results (condition-case e
                   (eval-scheme "(reverse test-results)")
                 (error (princ (format "M27IK-READBACK-ERROR: %S\n" e)) '())))
      (pass 0)
      (fail 0))
