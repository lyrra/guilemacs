...
Supports formats like #RGB, #RRGGBB, #RRRRGGGGBBBB, rgb(r,g,b), color names."
  (let ((spec (string-trim color-spec)))
    (cond
      ;; #RRRRGGGGBBBB format (16-bit per component, 48-bit color)
      ((and (string-prefix? "#" spec) (= (string-length spec) 13))
       (let ((r-str (substring spec 1 5))
             (g-str (substring spec 5 9))
             (b-str (substring spec 9 13)))
         (let ((r (string->number r-str 16))
               (g (string->number g-str 16))
               (b (string->number b-str 16)))
           (if (and r g b) (list r g b) #f))))
      ;; #RRGGBB format (8-bit per component, 24-bit color)
      ((and (string-prefix? "#" spec) (= (string-length spec) 7))
       (let ((r-str (substring spec 1 3))
         ...
         (let ((r (string->number r-str 16))
               ...
               (b (string->number b-str 16)))
           (if (and r g b)
               ;; Scale 8-bit to 16-bit (0-255 to 0-65535)
               (list (* r 257) (* g 257) (* b 257))
               #f))))
      ;; #RGB format (4-bit per component, shorthand)
           ...
           (if (= (length parts) 3)
               (let ((r (string->number (car parts)))
                 ...
                 (if (and r g b
                     ...
                     #f))
               #f))))
      ;; Named colors (basic set) - return 16-bit values
      ((string-ci=? spec "red") '(65535 0 0))
      ((string-ci=? spec "green") '(0 65535 0))
      ((string-ci=? spec "blue") '(0 0 65535))
      ((string-ci=? spec "white") '(65535 65535 65535))
      ((string-ci=? spec "black") '(0 0 0))
      ((string-ci=? spec "yellow") '(65535 65535 0))
      ((string-ci=? spec "cyan") '(0 65535 65535))
      ((string-ci=? spec "magenta") '(65535 0 65535))
      ((string-ci=? spec "gray") '(32896 32896 32896))
      ((string-ci=? spec "grey") '(32896 32896 32896))
      (else #f))))
