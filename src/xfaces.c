...
compare_fonts_by_sort_order (const void *v1, const void *v2)
{
  ...
  for (i = 0; i < FONT_SIZE_INDEX; i++)
	...
	{
	  if (STRINGP (val1))
	    result = STRINGP (val2) ? (scm_is_true (scm_string_equal_p (val1, val2)) ? 0 :
					 (scm_is_true (scm_string_less_p (val1, val2)) ? -1 : 1)) : -1;
	  else
                         ...
                         enum lface_attribute_index attr_filter)
{
  ...
  Lisp_Object attrs[LFACE_VECTOR_SIZE];
  struct face *base_face;
  bool multibyte_p = false; //STRING_MULTIBYTE (string);
