  ...
  (Lisp_Object frame)
{
  ...
  /* Use elisp vectors here because save_window_save accesses vector->contents[] */
  Lisp_Object tem = make_nil_elisp_vector (n_windows);
  data->saved_windows = tem;
  for (ptrdiff_t i = 0; i < n_windows; i++)
    ASET (tem, i, make_nil_elisp_vector (VECSIZE (struct saved_window)));
