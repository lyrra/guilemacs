...
key has been read inside `read-key-sequence'.  */);
  ...
  /* M32 imp-4 — the DEFSYM handles for the two names that left C for a
     Scheme declaration.  Their DEFVAR_* sites are gone, so these keep
     the generated Qsym #define and the defsym_name[] entry that
     make-docfile emits.  eval.c and keyboard.c read both through the
