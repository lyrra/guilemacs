      ...
      else if (EQ (key, QCscript))
	{
	  ...
	  if (CONSP (entry))
	      ...
	      if (CONSP (reps))
		{
	      ...
	      else if (VECTOR_OR_PSEUDOVECTORP (reps))
		{
...
{
  Lisp_Object val = make_nil_elisp_vector (8);
...
{
  Lisp_Object val = make_nil_elisp_vector (anchor->AnchorFormat + 1);
   ...
   Lisp_Object object)
{
  ...
  else if (VECTOR_OR_PSEUDOVECTORP (object))
    {
      ptrdiff_t ifrom, ito;
  ...
  Lisp_Object vec = make_nil_elisp_vector (len);
  for (ptrdiff_t i = 0; i < len; i++)
    {
      Lisp_Object g;
...
{
  Lisp_Object table = make_nil_elisp_vector (nelement);
  for (int i = 0; i < nelement; i++)
    {
      int j;
      for (j = 0; entry[i].names[j]; j++)
	continue;
      Lisp_Object elt = make_nil_elisp_vector (j + 1);
...
font_add_log (const char *action, Lisp_Object arg, Lisp_Object result)
{
  ...
  if (CONSP (result)
      && VECTOR_OR_PSEUDOVECTORP (XCAR (result))
      && ASIZE (XCAR (result)) > 0
  ...
  else if (VECTOR_OR_PSEUDOVECTORP (result))
    {
      result = Fcopy_sequence (result);
...
#include "font.x"
  ...
  Vfont_log_deferred = make_nil_elisp_vector (3);
