			...
			struct coding_detection_info *detect_info)
{
  ...
  while (rejected != CATEGORY_MASK_ISO)
	...
	check_extra_latin:
  if (!(VECTOR_OR_PSEUDOVECTORP (Vlatin_extra_code_table))
...
encode_coding_iso_2022 (struct coding_system *coding)
{
  ...
  if (VECTOR_OR_PSEUDOVECTORP (eol_type))
    eol_type = Qunix;
		       ...
		       struct coding_detection_info *detect_info)
{
  ...
  if (scm_is_true (scm_string_prefix_p (scm_from_utf8_string ("iso-8859-"), unwrap_emacs_string (SYMBOL_NAME (name)), SCM_INUM0, SCM_UNDEFINED, SCM_INUM0, SCM_UNDEFINED))
      || scm_is_true (scm_string_prefix_p (scm_from_utf8_string ("iso-latin-"), unwrap_emacs_string (SYMBOL_NAME (name)), SCM_INUM0, SCM_UNDEFINED, SCM_INUM0, SCM_UNDEFINED)))
    check_latin_extra = 1;
...
coding_inherit_eol_type (Lisp_Object coding_system, Lisp_Object parent)
{
  ...
  if (VECTOR_OR_PSEUDOVECTORP (eol_type))
      ...
      if (! NILP (parent))
	{
	  ...
	  if (!(VECTOR_OR_PSEUDOVECTORP (pspec_type)))
...
complement_process_encoding_system (Lisp_Object coding_system)
{
  ...
  for (i = 0; i < 3; i++)
	...
	coding_base = CODING_ATTR_BASE_NAME (attrs);
      if (NILP (eol_base)
          && !VECTOR_OR_PSEUDOVECTORP (AREF (spec, 2)))
...
detect_coding (struct coding_system *coding)
{
  ...
  if (! NILP (found))
    {
      int specified_eol = ((VECTOR_OR_PSEUDOVECTORP (eol_type)) ? EOL_SEEN_NONE
...
{
      if (FIXNUMP (trans) || VECTOR_OR_PSEUDOVECTORP (trans))
    {
      *nchars = 1;
      return trans;
    }
      ...
      while (buf < buf_end)
	{
	  ...
	  if (c >= 0)
	      ...
	      if (! NILP (trans))
		{
		  ...
		  if (FIXNUMP (trans))
		    c = XFIXNUM (trans);
	  else if (VECTOR_OR_PSEUDOVECTORP (trans))
		    {
	  ...
	  if (method != COMPOSITION_RELATIVE)
	    {
          ...
          if (VECTOR_OR_PSEUDOVECTORP (components))
		{
...
make_subsidiaries (Lisp_Object base)
{
  ...
  Lisp_Object subsidiaries = make_nil_elisp_vector (3);
  for (int i = 0; i < 3; i++)
    {
  ...
  (ptrdiff_t nargs, Lisp_Object *args)
{
  ...
  Lisp_Object attrs = make_nil_elisp_vector (coding_attr_last_index);
  ...
  if (EQ (coding_type, Qcharset))
	...
	goto short_args;
      ...
      if (VECTOR_OR_PSEUDOVECTORP (val))
	{
  ...
  if (NILP (eol_type))
      ...
      for (int i = 0; i < 3; i++)
	{
	  ...
	  this_spec = make_uninit_elisp_vector (3);
  ...
  Lisp_Object spec_vec = make_uninit_elisp_vector (3);
...
#include "coding.x"
  ...
  Vcoding_category_table = make_nil_elisp_vector (coding_category_max);
...
Only 128th through 159th elements have a meaning.  */);
  Vlatin_extra_code_table = make_nil_elisp_vector (256);
...
internal character representation.  */);
  ...
  memsetnil(args, coding_arg_undecided_max);
  Lisp_Object plist[] =
    {
      QCname,
