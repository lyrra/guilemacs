  ...
  (Lisp_Object keymap, Lisp_Object key, Lisp_Object accept_default)
{
  ...
  Lisp_Object new_key = make_elisp_vector (key_len, Qnil);
  ...
  if (!NILP (prefix))
      ...
      if (!NILP (tem))
	{
	  ...
	  if (STRINGP (prefix))
	    {
	      ptrdiff_t i_byte = 0;
	      Lisp_Object copy = make_nil_elisp_vector (SCHARS (prefix));
		 ...
		 bool keymap_p, bool mention_shadow)
{
  ...
  Lisp_Object kludge = make_nil_elisp_vector (1);
  ...
  for (int i = 0; ; i++)
	...
	{
	  Lisp_Object key = make_nil_elisp_vector (1);
...
that describe key bindings.  That is why the default is nil.  */);
  ...
  command_remapping_vector = make_elisp_vector (2, Qremap);
