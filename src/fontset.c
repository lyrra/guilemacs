...
#define RFONT_DEF_NEW(rfont_def, font_def)		\
  do {							\
    (rfont_def) = make_nil_elisp_vector (4);			\
...
fontset_get_font_group (Lisp_Object fontset, int c)
{
  ...
  if (!(VECTOR_OR_PSEUDOVECTORP (font_group)))
    return font_group;
		   ...
		   int charset_id, bool fallback)
{
  ...
  for (i = 0; i < ASIZE (vec); i++)
      ...
      if (! NILP (font_entity))
	{
	  ...
	  Lisp_Object new_vec = make_nil_elisp_vector (ASIZE (vec) + 1);
	  found_index++;
...
fontset_font (Lisp_Object fontset, int c, struct face *face, int id)
{
  ...
  if (VECTOR_OR_PSEUDOVECTORP (rfont_def))
    return rfont_def;
  if (NILP (rfont_def))
    FONTSET_SET (fontset, make_fixnum (c), make_fixnum (0));
  ...
  if (! EQ (base_fontset, Vdefault_fontset))
	...
	= fontset_find_font (FONTSET_DEFAULT (fontset), c, face, id, 0);
      if (VECTOR_OR_PSEUDOVECTORP (default_rfont_def))
...
font_for_char (struct face *face, int c, ptrdiff_t pos, Lisp_Object object)
{
  ...
  rfont_def = fontset_font (fontset, c, face, id);
  return ((VECTOR_OR_PSEUDOVECTORP (rfont_def))
...
fs_query_fontset (Lisp_Object name, int name_pattern)
{
  ...
  for (i = 0; i < ASIZE (Vfontset_table); i++)
	...
	continue;
	  ...
	  : scm_is_true (scm_string_ci_equal_p (name, this_name)))
	return i;
    }
  return -1;
}
      ...
      for (c = 0; c <= MAX_CHAR; )
	{
	  ...
	  if (VECTOR_OR_PSEUDOVECTORP (val))
	      ...
	      for (i = 0; ! NILP (realized[k][i]); i++)
		{
		  ...
		  if (! CONSP (val)
		      || !VECTOR_OR_PSEUDOVECTORP (XCDR (val)))
		    continue;
  ...
  (Lisp_Object name, Lisp_Object ch, Lisp_Object all)
{
  ...
  while (1)
	...
	if (VECTOR_OR_PSEUDOVECTORP (elt))
...
{
  Lisp_Object vec = make_nil_elisp_vector (3);
...
#include "fontset.x"
  ...
  Vfontset_table = make_nil_elisp_vector (32);
