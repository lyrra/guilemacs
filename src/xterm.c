...
#define NUM_ARGV 10
...

  {
	...
	if (!BASE_EQ (SYMBOL_FUNCTION (Qvendor_specific_keysyms), Qunbound))
	  {
  ...
  FOR_EACH_TAIL_SAFE (tem)
	...
	continue;
      ...
      if (!scm_is_true (call2 (intern ("string-equal-cstr"), XCAR (quit_keysym), scm_from_utf8_string (ServerVendor (dpy)))))
	continue;
...

  {
    ...
    if (STRINGP (value)
	&& (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("false")))
	    || scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("off")))))
  ...
  if (dpyinfo->visual == DefaultVisualOfScreen (dpyinfo->screen))
      ...
      if (dpyinfo->visual_info.class == PseudoColor)
	{
	  ...
	  if (STRINGP (value)
	      && (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("true")))
		  || scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("on")))))
 ...
 skip_xi_setup:
...

  {
    ...
    if (STRINGP (value))
      {
	if (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("callback"))))
	  dpyinfo->preferred_xim_style = STYLE_CALLBACK;
	else if (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("none"))))
	  dpyinfo->preferred_xim_style = STYLE_NONE;
	else if (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("overthespot"))))
	  dpyinfo->preferred_xim_style = STYLE_OVERTHESPOT;
	else if (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("offthespot"))))
	  dpyinfo->preferred_xim_style = STYLE_OFFTHESPOT;
	else if (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("root"))))
	  dpyinfo->preferred_xim_style = STYLE_ROOT;
#ifdef USE_GTK
	else if (scm_is_true (call2 (intern ("string-equal-cstr"), value, scm_from_utf8_string ("native"))))
