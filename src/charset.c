      ...
      if (! inhibit_load_charset_map)
	{
	  if (control_flag == 1)
	      ...
	      if (charset->method == CHARSET_METHOD_MAP)
		{
		  ...
		  vec = make_elisp_vector (n, make_fixnum (-1));
  ...
  (ptrdiff_t nargs, Lisp_Object *args)
{
  ...
  attrs = make_nil_elisp_vector (charset_attr_max);
			 ...
			 int code_offset)
{
  ...
  val = make_uninit_elisp_vector (8);
  for (i = 0; i < 8; i++)
  ...
  (Lisp_Object beg, Lisp_Object end, Lisp_Object table)
{
  ...
  charsets = make_nil_elisp_vector (charset_table_used);
  while (1)
    {
  ...
  (Lisp_Object str, Lisp_Object table)
{
  ...
  Lisp_Object charsets = make_nil_elisp_vector (charset_table_used);
