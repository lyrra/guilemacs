  ...
  (let* ((files (map (lambda (file)
         ...
         (args (append
                ...
                (apply append (map (lambda (file)
                                     (list "-l" file))
                                   files))
                '("--batch" "--eval" "(ert-run-tests-batch-and-exit (quote (not (or (tag :expensive-test) (tag :unstable) (tag :nativecomp)))))"))))
    (format #t "Running test files: ~a~%" files)
    (format #t "running ===> ~a ~a~%" cmd args)
    (let* ((port (pipe))
           (pid (spawn cmd args
                       #:error (cdr port)
                       #:search-path? #f))
      ...
      (do ((line (read-line (car port)) (read-line (car port))))
          ((or done (eof-object? line)))
        (format #t "~a >>> ~a~%" (+ tot %total-passed-tests) line)
        (cond
         ;; we wont get eof at process exit (defunct), so need to detect when done
